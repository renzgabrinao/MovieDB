	...
	useEffect(() => {
		const loadMovies = async () => {
			...
			response = await fetch(
				`https://api.themoviedb.org/3/movie/${search.filter}?api_key=${MOVIE_DB_API_KEY}&language=en-US`
			);

			if (search.search !== '') {
				response = await fetch(
					`https://api.themoviedb.org/3/search/movie?api_key=${MOVIE_DB_API_KEY}&language=en-US&query=${search.search}&page=1&include_adult=false`
				);
			}
	...
	return (
		<>
			<div className="filter">
				...
				<input
					className="search-bar filter-item"
					type="text"
					name="search"
					value={search.search}
